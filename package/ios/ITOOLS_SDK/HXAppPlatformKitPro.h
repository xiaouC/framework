 ...
 * \param   appId           合作时申请到的应用id
 * \param   appKey          合作时申请到的应用id对应的key
 \param   useUnipayWap    YES时使用银联wap,NO时使用银联SDK，默认值为NO
 */
+ (void)setAppId:(int)appId appKey:(NSString *)appKey useUnipayWap:(BOOL)useUnipayWap;
...
/*! 设置支付宝回调所需URL Scheme
 * \param   alipayURLScheme     此alipayURLScheme需在工程中配置相应的URL Scheme
 */
