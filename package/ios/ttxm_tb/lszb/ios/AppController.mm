...
{
    NSLog(@"Init callback payload: %d, %s", result, jsonStr);
    if (result) {
        NSLog(@"Init successed.");
    } else {
        NSLog(@"Init failed.");
        BlockAlertView *alertView = [BlockAlertView new];
        [alertView showAlertWithTitle:@"" msg:@"SDK 初始化失败" callbackBlock:^(int btnIndex){
