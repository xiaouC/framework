...
_rmessage_new(lua_State *L) {
	struct pbc_env * env = (struct pbc_env*)checkuserdata(L,1);
	const char * typename_ = luaL_checkstring(L,2);
	struct pbc_slice slice;
	if (lua_isstring(L,3)) {
		size_t sz = 0;
	...
	struct pbc_rmessage * m = pbc_rmessage_new(env, typename_, &slice);
	if (m==NULL)
		return 0;
...
_env_type(lua_State *L) {
	...
	const char * typename_ = luaL_checkstring(L,2);
	if (lua_isnil(L,3)) {
		int ret = pbc_type(env, typename_, NULL, NULL);
		lua_pushboolean(L,ret);
		return 1;
	}
	const char * key = luaL_checkstring(L,3);
	const char * type = NULL;
	int ret = pbc_type(env, typename_, key, &type);
...
_wmessage_new(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char * typename_ = luaL_checkstring(L,2);
	struct pbc_wmessage * ret = pbc_wmessage_new(env, typename_);
...
static void
push_value(lua_State *L, int type, const char * typename_, union pbc_value *v) {
	switch(type) {
	case PBC_INT:
	...
	default:
		luaL_error(L, "Unknown type %s", typename_);
		break;
	}
}
 ...
 */
static void
decode_cb(void *ud, int type, const char * typename_, union pbc_value *v, int id, const char *key) {
	...
	if (type & PBC_REPEATED) {
		push_value(L, type & ~PBC_REPEATED, typename_, v);
...
_gc(lua_State *L) {
	...
	obj->size_msg = 0;
	obj->cap_msg = 4;
	obj->pat = (pbc_pattern **)malloc(obj->cap_pat * sizeof(struct pbc_pattern *));
	obj->msg = (pbc_rmessage **)malloc(obj->cap_msg * sizeof(struct pbc_rmessage *));
